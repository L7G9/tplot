...
class TagCreateViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse(
                "age_timelines:tag-add",
                kwargs={'age_timeline_id': self.user0_age_timeline_id}
            )
        )
        self.assertRedirects(
            response,
            f"/accounts/login/?next=/timelines/age/{self.user0_age_timeline_id}/tag/add/"
        )
    ...
    def test_view_url_exists_at_desired_location(self):
        ...
        response = self.client.get(
            f"/timelines/age/{self.user0_age_timeline_id}/tag/add/"
        )
    ...
    def test_uses_correct_template(self):
        ...
        self.assertTemplateUsed(
            response,
            "age_timelines/tag_add_form.html"
        )
