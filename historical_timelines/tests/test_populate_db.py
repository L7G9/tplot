...
class PopulateDBTest(TestCase):
    ...
    def test_populate_db_tables(self):
        ...
        self.assertEqual(
            len(HistoricalTimeline.objects.all()), total_timelines
        )
        self.assertEqual(
            len(HistoricalEvent.objects.all()),
    ...
    def test_populate_db_output(self):
        ...
        self.assertEqual(
            len(users[0]["historical_timelines"]), self.TIMELINES_PER_USER
        )
        self.assertEqual(
            len(users[0]["historical_timelines"][0]["historical_event_ids"]),
            self.EVENTS_PER_TIMELINE,
        )
        self.assertEqual(
            len(users[0]["historical_timelines"][0]["tag_ids"]),
            self.TAGS_PER_TIMELINE,
        )
        self.assertEqual(
            len(users[0]["historical_timelines"][0]["event_area_ids"]),
            self.EVENT_AREAS_PER_TIMELINE,
        )
    ...
    def test_populate_db_none(self):
        ...
        self.assertEqual(len(HistoricalTimeline.objects.all()), 0)
        self.assertEqual(len(HistoricalEvent.objects.all()), 0)
