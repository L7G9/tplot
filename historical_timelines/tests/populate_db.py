...
def populate_db(
    ...
    for user_index in range(user_count):
        ...
        for timeline_index in range(timelines_per_user):
            timeline = HistoricalTimeline.objects.create(
                user=user, title=f"HistoricalTimeline{timeline_index}"
            )
            historical_timeline_ids = {
                "id": timeline.id,
                "historical_event_ids": [],
                "tag_ids": [],
                "event_area_ids": [],
            }
            user_ids["historical_timelines"].append(historical_timeline_ids)
            ...
            for event_index in range(events_per_timeline):
                ...
                historical_timeline_ids["historical_event_ids"].append(
                    event.id
                )
            ...
            for tag_index in range(tags_per_timeline):
                ...
                historical_timeline_ids["tag_ids"].append(tag.id)
            ...
            for area_index in range(areas_per_timeline):
                ...
                historical_timeline_ids["event_area_ids"].append(event_area.id)
