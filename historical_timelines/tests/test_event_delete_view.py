...
class HistoricalEventDeleteViewTest(TestCase):
    USER_COUNT = 2
    ...
    def test_view_url_exists_at_desired_location(self):
        ...
        response = self.client.get(
            f"/timelines/historical/{timeline_id}/event/{event_id}/delete/"
        )
    ...
    def test_view_url_accessible_by_name(self):
        ...
        response = self.client.get(
            reverse(
                "historical_timelines:event-delete",
                kwargs={
    ...
    def test_uses_correct_template(self):
        ...
        self.assertTemplateUsed(
            response, "historical_timelines/event_delete.html"
        )
    ...
    def test_historical_event_deleted(self):
        self.client.login(
            username=self.user0.username, password=self.user0_password
        )
        ...
        historical_events = HistoricalEvent.objects.filter(
            historical_timeline=self.user0_hist_timeline_id
        )
        expected_historical_event_count = self.EVENTS_PER_TIMELINE - 1
        self.assertEqual(
            len(historical_events), expected_historical_event_count
        )

    def test_redirect_after_historical_event_deleted(self):
        self.client.login(
            username=self.user0.username, password=self.user0_password
        )
        response = self.client.post(
